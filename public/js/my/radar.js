let radarChart = (function () {
    ...
    function draw(cluster) {
        ...
        for (let i = 0; i < value_arr.length; i++) {
            for (let j = 0; j < 5; j++) {
                varianceArr[j] += Math.pow(average_value[j] - value_arr[i][j], 2) / value_arr.length;
            }
        }
        let aveScale = d3.scaleLinear().domain(d3.extent(average_value)).range([0.3, 0.8])
        let varianceScale = d3.scaleLinear().domain(d3.extent(varianceArr)).range([0.05, 0.2])
        let area_variance = [];
        for (let i = 0; i < varianceArr.length; i++) {
            let tmp_dict = {}
            tmp_dict['inner'] = aveScale(average_value[i]) - varianceScale(varianceArr[i])
            tmp_dict['outer'] = aveScale(average_value[i]) + varianceScale(varianceArr[i])
            area_variance.push(tmp_dict)
        }
        let data = [[], []];
        for (let i = 0; i < average_value.length; i++) {
            data[0].push({ axis: variable.time_arr[i], value: aveScale(average_value[i]) })
        }
        ...
        console.log('area_variance: ', area_variance);
        let varianceArea = g.append("path")
            .attr("d", radarArea(area_variance))
            .style("fill", '#FF9341')
